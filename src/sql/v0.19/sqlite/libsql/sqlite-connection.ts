...
class LS3Taste implements TasteHandler {
  readonly taste = "libsql" as const;
...
const onceSQLiteConnections = new KeyedResolvOnce<Client>();
export class V0_19LS3Connection extends Sqlite3Connection {
  ...
  constructor(_sthis: SuperThis, url: URI, opts: Partial<SQLOpts>) {
    const sthis = ensureSuperLog(_sthis, "V0_19LS3Connection", { url });
    super(sthis, url, ensureSQLOpts(sthis, url, opts), new LS3Taste());
  }
