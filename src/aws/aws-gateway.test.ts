...
describe("AWSGateway", () => {
  ...
  it("should initialize and perform basic operations", async () => {
    ...
    db = fireproof("aws-test-db" + sthis.nextId().str, config);

    const dataStore = (await db.crdt.blockstore.loader?.carStore()) as bs.DataStore;
    // Assert that loader has ebOpts.store.stores
    expect(dataStore).toBeDefined();
    if (!dataStore) {
      throw new Error("Loader is not defined");
    }
    // expect(dataStore.ebOpts).toBeDefined();
    // expect(dataStore.ebOpts.store).toBeDefined();
    // expect(dataStore.ebOpts.store.stores).toBeDefined();
    // if (!dataStore.ebOpts.store.stores) {
