...
class CFWSRoom implements WSRoom {
  ...
  removeConn(conn: QSId): void {
    if (!this.isWebsocket) {
      const idx = this.notWebSockets.findIndex((i) => qsidEqual(i.conn, conn));
      if (idx >= 0) {
        this.notWebSockets.splice(idx, 1);
      }
      return;
    }
    const found = this.getConns().find((i) => qsidEqual(i.conn, conn));
    if (!found) {
      return;
    }
  ...
  isConnected(msg: MsgBase): msg is MsgWithConn<MsgBase> {
    ...
    return !!this.getConns().find((i) => qsidEqual(i.conn, msg.conn));
    // // eslint-disable-next-line no-console
