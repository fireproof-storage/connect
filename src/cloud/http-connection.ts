...
import { selectRandom } from "./msg-request.js";
import { MsgBase, MsgIsResOpen, ReqOpen, ResOpen, buildErrorMsg, MsgerParams } from "./msg-types.js";
import { ExchangedGestalt, MsgConnection, OnMsgFn, timeout, UnReg } from "./msger.js";
...
export class HttpConnection implements MsgConnection {
  ...
  constructor(sthis: SuperThis, reqOpen: ReqOpen | undefined, uris: URI[], msgP: MsgerParams, exGestalt: ExchangedGestalt) {
    this.sthis = sthis;
  ...
  async request<Q extends MsgBase, S extends MsgBase>(req: Q, _opts: RequestOpts): Promise<WithErrorMsg<S>> {
    ...
    const rRes = await exception2Result(() => timeout(this.msgParam.timeout, fetch(url.toString(), {
      method: "PUT",
      headers,
      body: rReqBody.Ok(),
    })));
