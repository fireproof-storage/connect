...
describe("Hello World worker", () => {
  it("binary put", async () => {
    ...
    const id = myEnv.CFTestStorage.idFromName("test");
    const dobj = myEnv.CFTestStorage.get(id);
    await dobj.put("test", base64pad.encode(ref));
    const res = base64pad.decode((await dobj.get("test")) || "");
  ...
  it("responds with Hello World!", async () => {
    ...
    expect(await response.text()).toBe("Hello World!test-db");
  });
});
