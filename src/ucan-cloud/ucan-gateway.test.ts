...
import { AgentDataExport, AgentMeta, DelegationMeta } from "@web3-storage/access/agent";
...
async function authorizeW3({
  ...
  const raw: AgentDataExport = {
    ...
    spaces: new Map(),
    delegations: new Map([exportDel(delegation), exportDel(attestation)]),
  };

  await store.save(raw);
}

function uint8ArrayToArrayBuffer(array: Uint8Array) {
