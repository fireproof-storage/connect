...
import { bs, ensureLogger, PARAM, SuperThis } from "@fireproof/core";
...
export interface Transport {
  ...
  put(url: URI, frag: FragmentData): Promise<bs.VoidResult>;
  get(url: URI, frag: Fragment): Promise<Result<FragmentData>>;
  delete(url: URI, frag: Fragment): Promise<bs.VoidResult>;
  close(url: URI): Promise<bs.VoidResult>;
}
  ...
  slicer(url: URI, body: Uint8Array): Promise<bs.VoidResult>[] {
    ...
    for (let ofs = 0; ofs < body.length; ofs += blocksize) {
      ...
      if (block.length > fragSize) {
        throw this.logger
          .Error()
          .Uint64("block", block.length)
          .Uint64("fragSize", fragSize)
          .Msg("Block size to big")
          .AsError();
      }
      ops.push(
